...
foreach ($cfgPath in $ngrokConfigPaths) {
    if (Test-Path $cfgPath) {
        ...
        $needsFix = $false
        # Check for v3 config format (version: 3 or agent: block)
        if ($cfgContent -match 'version:\s*"?3"?' -or $cfgContent -match '(?m)^agent:') {
            $needsFix = $true
        }
        if ($needsFix) {
            Write-Info "Fixing ngrok config format at $cfgPath"
            # Extract authtoken from either format
            $extractedToken = $null
...
The Revit MCP Server connects Claude to Autodesk Revit 2024/2025/2026/2027 via the Model Context Protocol. It provides 45 tools for building design, model editing, structural systems, MEP, documentation, and analysis.
